...
folder = 'experiments/imagenet/random'
if not os.path.isdir(folder):
    os.mkdir(folder)

folder = 'experiments/imagenet/random/tgv'
if not os.path.isdir(folder):
    os.mkdir(folder)

folder = 'experiments/imagenet/random/tgv/inpainting'
if not os.path.isdir(folder):
    os.mkdir(folder)
...
for i in range(1,101):
    original = mp.imread('imsource/imagenet/img_'+str(i)+'.png')
    mask = mp.imread('imsource/imagenet/mask.png')
    mask = mask.astype(np.int32)
    res = mp.tgv_recon(imname='imsource/imagenet/img_'+str(i),u0=original,mask=mask,check=niter, **fixpars)
