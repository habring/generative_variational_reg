...
class cp_conv_nlayers(object):
    ...
    def fwd(self,c,D):
        ...
        for i in range(n-1):
            ...
            for ii in range(self.K[n-i-2].nf):
    ...
    def grad_D(self,c,D,u):
        ...
        for i in range(1,n):
            gradDi = np.zeros([*self.K[i].indim2,self.K[i-1].nf])
            for ii in range(self.K[i-1].nf):
...
__kernel void malplus4D(__global float *b , __global const float *a1 , const float s1 , __global const float *a2, const float s2) {
    ...
    for (int x = 0; x < ksz; ++x) {
        i = x + y*ksz + z*ksz*ksz + layer_out*ksz*ksz*nf;
        b[i] = s1*a1[i] + s2*a2[i];
    }
}
...
__kernel void conv_cD_fwd(__global float *u , __global const float *c, __global const float *D , const int nf , const int nfout , const int ksz , const int stride , const int nx , const int ny) {
    INIT_INDICES
    ...
    for (int ff = 0; ff < nf; ++ff)
    {
        for (int k = (int)ceil( ((float)(x_conv-ksz+1))/((float)stride) ); x_conv - k*stride > -1 ; ++k)
        {
            for (int l = (int)ceil( (float)((y_conv-ksz+1)/(float)stride) ); y_conv - l*stride > -1; ++l)
            {
                val = val + c[ff*nx*ny + l*nx + k] * D[z*ksz*ksz*nf + ff*ksz*ksz + (y_conv-l*stride)*ksz + (x_conv-k*stride)];
            }
        }
    }
    u[i] = val;
...
__kernel void conv_adj(__global float *c , __global const float *u , __global const float *D , const int nf , const int ksz , const int stride , const int Nx, const int Ny) {
    int nx = get_global_size(0);
    int ny = get_global_size(1);
    int x = get_global_id(0);
    int y = get_global_id(1);
...
__kernel void conv_adj_multifilter(__global float *c , __global const float *u , __global const float *D , const int nf_c , const int nf_u , const int ksz , const int stride , const int Nx, const int Ny) {
    int nx = get_global_size(0);
    int ny = get_global_size(1);
    int x = get_global_id(0);
    int y = get_global_id(1);
    ...
    for(int layer = 0; layer < nf_u; ++layer) {
        for(int i = imin; i+ksz-1-x*stride < ksz && i < Nx; ++i) {
            for(int j = jmin; j+ksz-1-y*stride < ksz && j < Ny; ++j) {

                val = val + D[layer*ksz*ksz*nf_c + z*ksz*ksz + (j+ksz-1-y*stride)*ksz + (i+ksz-1-x*stride)] * u[layer*Nx*Ny + j*Nx + i];
            }
...
__kernel void conv_adj_kernel(__global float *D , __global const float *u , __global const float *c , const int layer , const int nf , const int stride , const int Nx, const int Ny , const int nx, const int ny) {
    int ksz = get_global_size(0);
    int x = get_global_id(0);
    int y = get_global_id(1);
...
__kernel void l1_prox_c(__global float *c , const float tau , const int nf) {
    INIT_INDICES
...
__kernel void D_prox_l2_leq1(__global float *D, const int ksz) {
    ...
    int nf = get_global_size(0);
    int nf_out = get_global_size(1);

    int z = get_global_id(0);
    int layer_out = get_global_id(1);

    int i0 = z*ksz*ksz + layer_out*ksz*ksz*nf;
...
def gen_reg(**par_in):
    ...
    par.niter = 5000
    par.n = 3 #number of layers
    par.nf = [8,8,8] #number of filters
    ...
    par.pcoeff = [(1.0-par.nu)/(min(par.nu,1.0-par.nu)) for i in range(par.n)]
    ...
    def conv_cD_fwd_layer(u , c , D , nf , ksz , stride , nfout=1):
        return prgs.conv_cD_fwd(u.queue, u.shape, None , u.data, c.data, D.data , np.int32(nf) , np.int32(nfout) , np.int32(ksz) , np.int32(stride) , np.int32(c.shape[0]) , np.int32(c.shape[1]))
    ...
    def conv_cD_fwd(conv , u , c , c_list , D):
        ...
        for layer in range(n-1):
            conv_cD_fwd_layer(c_list[n-2-layer],c_list[n-1-layer], D[n-1-layer] , conv.K[n-1-layer].nf , conv.K[n-1-layer].ksz , conv.K[n-1-layer].stride , conv.K[n-1-layer-1].nf)

        D_layer = D[0]
        conv_cD_fwd_layer(u , c_list[0] , D_layer , par.nf[0] , par.ksz[0] , par.stride[0] , 1)
    ...
    def conv_adj(c , u , D , nf , ksz , stride , Nx , Ny):
        return prgs.conv_adj(c.queue, c.shape, None , c.data, u.data, D.data , np.int32(nf) , np.int32(ksz) , np.int32(stride) , np.int32(Nx) , np.int32(Ny))

    
    def conv_adj_multifilter(c , u , D , nf_c , nf_u , ksz , stride , Nx , Ny):
        return prgs.conv_adj_multifilter(c.queue, c.shape, None , c.data , u.data , D.data , np.int32(nf_c) , np.int32(nf_u) , np.int32(ksz) , np.int32(stride) , np.int32(Nx) , np.int32(Ny))


    def conv_adj_kernel(D , u , c , layer , nf , stride , Nx , Ny ,nx , ny):
        return prgs.conv_adj_kernel(D.queue, D.shape, None , D.data , u.data , c.data , np.int32(layer) , np.int32(nf) , np.int32(stride) , np.int32(Nx) , np.int32(Ny) , np.int32(nx) , np.int32(ny))

    def conv_grad_c(conv , gradc , c_list , u , D):
        n = len(D)
        if(n>1):
            conv_adj(c_list[0] , u , D[0] , conv.K[0].nf , conv.K[0].ksz , conv.K[0].stride , conv.K[0].outdim[0] , conv.K[0].outdim[1])
        else:
            conv_adj(gradc , u , D[0] , conv.K[0].nf , conv.K[0].ksz , conv.K[0].stride , conv.K[0].outdim[0] , conv.K[0].outdim[1])



        for i in range(1,n):
            if( i < n-1 ):
                conv_adj_multifilter(c_list[i] , c_list[i-1] , D[i] , conv.K[i].nf , conv.K[i-1].nf , conv.K[i].ksz , conv.K[i].stride , conv.K[i].outdim[0] , conv.K[i].outdim[1])
            else:
                conv_adj_multifilter(gradc , c_list[i-1] , D[i] , conv.K[i].nf , conv.K[i-1].nf , conv.K[i].ksz , conv.K[i].stride , conv.K[i].outdim[0] , conv.K[i].outdim[1])
    ...
    def conv_grad_D(conv , gradD , c_list , c_list_tmp , D , D_tmp , u):
        ...
        #copmute grad on all layers
        conv_adj_kernel(gradD[0] , u , c_list[0] , 0 , conv.K[0].nf , conv.K[0].stride , u.shape[0] , u.shape[1] , conv.K[0].indim[0] , conv.K[0].indim[1])

        conv_adj(c_list_tmp[0] , u , D[0] , conv.K[0].nf , conv.K[0].ksz , conv.K[0].stride , conv.K[0].outdim[0] , conv.K[0].outdim[1])


        for i in range(1,n):

            for ii in range(conv.K[i-1].nf):
                conv_adj_kernel(D_tmp[i], c_list_tmp[i-1] , c_list[i] , ii , conv.K[i].nf , conv.K[i].stride , conv.K[i].outdim[0] , conv.K[i].outdim[1] , conv.K[i].indim[0] , conv.K[i].indim[1])
                gradD[i][:,:,:,ii] = D_tmp[i]

            conv_adj_multifilter(c_list_tmp[i] , c_list_tmp[i-1] , D[i] , conv.K[i].nf , conv.K[i-1].nf , conv.K[i].ksz , conv.K[i].stride , conv.K[i].outdim[0] , conv.K[i].outdim[1])
    ...
    if par.application == 'deconvolution':
        ...
        def compute_energy(conv , u , v, v_tmp , g , v_tveps , c , imsyn_list , D):
            ...
            conv_cD_fwd_layer(v , c[0] , D[0] , conv.K[0].nf , conv.K[0].ksz , conv.K[0].stride , 1)
            grad_fwd_diff(g , u , v)
            ...
            for i in range(par.n):
                ...
                if i>0:
                    # compute convolution
                    conv_cD_fwd_layer(imsyn_list[i] , c[i] , D[i] , conv.K[i].nf , conv.K[i].ksz , conv.K[i].stride , conv.K[i-1].nf)
    ...
    def compute_grad_c(gradc , conv , u , v , guc , gradu , c , imsyn_list , D):
        ...
        conv_adj(gradc[0] , gradu , D[0] , conv.K[0].nf , conv.K[0].ksz , conv.K[0].stride , conv.K[0].outdim[0] , conv.K[0].outdim[1])
        ...
        for i in range(1,par.n):
            ...
            conv_adj_multifilter(gradc[i] , imsyn_list[i] , D[i] , conv.K[i].nf , conv.K[i-1].nf , conv.K[i].ksz , conv.K[i].stride , conv.K[i].outdim[0] , conv.K[i].outdim[1])
    ...
    def compute_grad_D(gradD , conv , u , v , guc , gradu , c, imsyn_list , D , D_tmp):
        ...
        grad_fwd_diff(guc , v , u)
        grad_adj_l1vec_grad(gradu , guc , par.eps_TV , par.ptv/(N*M))
        conv_adj_kernel(gradD[0] , gradu , c[0] , 0 , conv.K[0].nf , conv.K[0].stride , gradu.shape[0] , gradu.shape[1] , conv.K[0].indim[0] , conv.K[0].indim[1])
        ...
        for i in range(1,par.n):
            ...
            for ii in range(conv.K[i-1].nf):
                conv_adj_kernel(D_tmp[i], imsyn_list[i] , c[i] , ii , conv.K[i].nf , conv.K[i].stride , conv.K[i].outdim[0] , conv.K[i].outdim[1] , conv.K[i].indim[0] , conv.K[i].indim[1])
                gradD[i][:,:,:,ii] = D_tmp[i]
    ...
    def prox_c(c , tau , nf):
        return prgs.l1_prox_c(c.queue , c.shape , None , c.data , np.float32(tau) , np.int32(nf))
    ...
    M_out = M
    for i in range(par.n):
        Ki = cp_conv([N_out,M_out,par.ksz[i],par.nf[i]],stride=par.stride[i])
        K_list.append(Ki)
    ...
    # atoms/kernels
    if ( len(par.D_init)==0 ):
        D0 = np.random.rand(par.ksz[0],par.ksz[0],par.nf[0])
        D0 -= D0.sum(axis=(0,1),keepdims=True)/(par.ksz[0]*par.ksz[0])
        D0 /= np.maximum(1.0, np.sqrt(np.square(D0).sum(axis=(0,1),keepdims=True)) )
        D_init = [D0]
        for i in range(1,par.n):
            Di = np.zeros([par.ksz[i],par.ksz[i],par.nf[i],par.nf[i-1]])
            for j in range(par.nf[i]):
        ...
        for i in range(len(par.D_init), par.n):
            Di = np.zeros([par.ksz[i],par.ksz[i],par.nf[i],par.nf[i-1]])
            for j in range(par.nf[i]):
                Di[:,:,j,j] = np.random.rand(par.ksz[i],par.ksz[i])
            #Di -= Di.sum(axis=(0,1),keepdims=True)/(par.ksz[0]*par.ksz[0])
    ...
    if ( len(par.c_init)>0 ):
        ...
        for i in range(len(par.c_init), par.n):
            conv_adj_multifilter(cd[i] , cd[i-1] , Dd[i] , conv.K[i].nf , conv.K[i-1].nf , par.ksz[i] , par.stride[i] , conv.K[i].outdim[0] , conv.K[i].outdim[1])

        if par.init_splitting_zero:
            for i in range(par.n-1):
                conv_cD_fwd_layer(cd[par.n-2-i], cd[par.n-1-i], Dd[par.n-1-i] , conv.K[par.n-1-i].nf , conv.K[par.n-1-i].ksz , conv.K[par.n-1-i].stride , conv.K[par.n-2-i].nf)

            conv_cD_fwd_layer(vd , cd[0] , Dd[0] , par.nf[0] , par.ksz[0] , par.stride[0] , 1)
    ...
    if par.show_every:
        ...
        conv_cD_fwd_layer(textd , cd[0] , Dd[0] , par.nf[0] , par.ksz[0] , par.stride[0] , 1)
        u_show = ud.get()
        text = textd.get()
        uu0 = res.u0
        imnormalize(uu0)
        ...
        for i in range(par.n):
            ...
            D.append(Dd[i].get())
        for i in range(par.n-1):
            conv_cD_fwd_layer(cd_tmp[par.n-2-i], cd[par.n-1-i], Dd[par.n-1-i] , conv.K[par.n-1-i].nf , conv.K[par.n-1-i].ksz , conv.K[par.n-1-i].stride , conv.K[par.n-2-i].nf)
            imsyn_list = [cd_tmp[par.n-2-i].get()] + imsyn_list
    ...
    while (k<par.niter and not(opt)):
        ...
        while( bt<=10 and not desc ):
            ...
            for i in range(par.n):
                ...
                #prox
                prox_c(cd[i] , tau_c/(par.pcoeff[i]/(cd[i].shape[0]*cd[i].shape[1])) , par.nf[i])
...
def gen_reg_successively(**par_in):
    ...
    par.ksz = [8,8,8] #kernel size
    par.stride = [1,2,2] #kernel size
    par.niter = [500 for i in range(par.n-1)] + [4000] #par.niter[i] = number of iterations with (i+1) layer net
    par.communication = 0 #For all D[i] with i > par.communication, we enforce D[i][:,:,k,l]=0 for k \neq l
    par.nu = 0.95
    par.ld = 100
    par.splitting = 2e3
    ...
    print('Final run: '+ str(n)+' layer net')
    res = gen_reg(**par.__dict__, **data_in.__dict__)
